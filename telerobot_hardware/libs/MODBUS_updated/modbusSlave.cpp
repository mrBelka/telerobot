...
void modbusSlave::run(void)
{
	...
	if(funcType == WRITE_MULTI_AO)
	{
	
		word reg = (_msg[2] << 8) | (_msg[3]);
		word count = (_msg[4] << 8) | (_msg[5]);
		for(int i=0;i<count;i++)
		{
			word val = (_msg[7 + 2*i] << 8) | _msg[8 + 2*i];
			_device->set(reg + 40001 + i, val);
		}
